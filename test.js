...
eq(mes.map(x => x.p), [2026035, 2026039, 2026044, 2026048, 2026053], 'month-end weeks Aug..Dec 2026 = week of each month\'s last Thursday (8/27 9/24 10/29 11/26 12/31), consistent with Thursday attribution');
...
/* ---------- 锁量顺序分配:手拍优先、自动量从前往后、后面被砍、组预算、手拍超预算 ---------- */
(function () {
  const E = function (p, si, fixed, g) { return { p: p, si: si, fixed: !!fixed, g: g || '' }; };
  const r1 = C.lockSeqAlloc([E(1, 100), E(2, 100), E(3, 100), E(4, 100)], { total: 250, step: 5 });
  eq(r1.alloc.join('/'), '100/100/50/0', 'seqAlloc: earlier weeks served first, later ones cut');
  eq(r1.cutAt, 3, 'seqAlloc: first cut at p=3');
  eq(r1.used, 250, 'seqAlloc: uses exactly the budget');
  /* 2026-09-02 第二轮审计:手拍按时间顺序占预算(用户口径「后面月份自动调减,之前调过的不动」),
     不再不论时间先后一次性预扣——拍晚期的 SI 不会把更早的自动 SI 清 0,超支记 manOver */
  const r2 = C.lockSeqAlloc([E(1, 100), E(2, 100), E(3, 300, true), E(4, 100)], { total: 350, step: 5 });
  eq(r2.alloc.join('/'), '100/100/300/0', 'seqAlloc: earlier autos keep their rule qty, later manual tap stays as-is');
  eq(r2.manOver, true, 'seqAlloc: later manual tap pushes total over budget → manOver');
  const r2b = C.lockSeqAlloc([E(1, 300, true), E(2, 100), E(3, 100)], { total: 350, step: 5 });
  eq(r2b.alloc.join('/'), '300/50/0', 'seqAlloc: early manual tap first, later autos get the leftover');
  eq([r2b.manOver, r2b.cutAt], [false, 2], 'seqAlloc: early tap within budget → no manOver, cut at p=2');
  const r3 = C.lockSeqAlloc([E(1, 100), E(2, 400, true)], { total: 300, step: 5 });
  eq(r3.alloc.join('/'), '100/400', 'seqAlloc: manual exceeding budget stays; earlier auto untouched');
  eq(r3.manOver, true, 'seqAlloc: manOver flagged');
  const r4 = C.lockSeqAlloc([E(1, 100, false, 'CO'), E(1, 100, false, 'MX'), E(2, 100, false, 'CO')], { total: 1000, by: { CO: 120 }, step: 5, groupsOf: function (e) { return e.g === 'CO' ? ['CO'] : []; } });
  eq(r4.alloc.join('/'), '100/100/20', 'seqAlloc: group budget (CO quota 120) caps CO entries, MX untouched');
  eq(C.lockSeqAlloc([E(1, 100), E(2, 100)], { total: 1000, step: 5 }).cutAt, null, 'seqAlloc: within budget → no cut');
  eq(C.lockSeqAlloc([E(1, 103)], { total: 1000, step: 5 }).alloc[0], 100, 'seqAlloc: auto floored to step');
  /* 重链 */
  const mkRow = function (y, m, so, si, inv) { return ['L', 'P', 'P', 'F', 'X', 'A', 'FSD', y, 'Q' + Math.ceil(m / 3), m, so, si, inv, '', '', '', '', 'M']; };
  const rows = [mkRow(2027, 1, 100, 200, 300), mkRow(2027, 2, 100, 0, 200)];
  rows[0][11] = 50;   // 分配后 1 月 SI 200→50
  C.rechainMonthRows(rows, [0, 1], 200);   // 期初 200(改前 300−200+100)
  eq(rows[0][12], 150, 'rechain: Jan end = 200+50−100');
  eq(rows[1][12], 50, 'rechain: Feb end = 150+0−100');
  const rows2 = [mkRow(2027, 1, 100, 0, 0)];
  C.rechainMonthRows(rows2, [0], 30);
  eq(rows2[0][10] + '/' + rows2[0][12], '30/0', 'rechain: SO capped by 期初+SI');
})();
...
(function () {
  ...
  eq(C.monthEndPeriodOf(2026, 8), C.mkPeriod(2026, 35), 'monthEnd: Aug 2026 ends at W35 (last Thursday 8/27), not W36 (Thursday 9/3)');
  eq(C.monthEndPeriodOf(2026, 12), C.mkPeriod(2026, 53), 'monthEnd: Dec 2026 ends at W53 (Thursday 12/31)');
  eq(C.monthEndPeriodOf(2027, 1), C.mkPeriod(2027, 4), 'monthEnd: Jan 2027 ends at W4 (last Thursday 1/28)');
  const prof = C.thisYearProfileOf(u, sim, maxP);
  ok(prof && prof.n === 12, 'thisYearProfile: 12 months (8 actual + 4 simulated)');
  eq(prof.m[8].inv, 1000, 'thisYearProfile: August month-end inventory from actual data');
  eq(prof.m[12].inv, 1500, 'thisYearProfile: December month-end inventory from simulation');
  eq(prof.m[8].dos, Math.round(1000 / (70 * 4 / 28)), 'thisYearProfile: Aug DOS = 1000 ÷ (280/28) = 100');
  eq(prof.m[10].dos, Math.round(1500 / (70 * 5 / 35)), 'thisYearProfile: Oct (5 weeks) DOS = 1500 ÷ (350/35) = 150');
  eq(C.thisYearProfileOf({ periods: [], weeks: new Map() }, [], maxP), null, 'thisYearProfile: no data → null');
})();

/* ---------- PSI 类型映射:SO1/Sell Out1 → so1(独立,不相加);SO TTL/Sell Out(TTL)/SO → so;导入报告列出映射 ---------- */
(function () {
  const hdr = ['Management Country/Region', 'Management Account Name(D)', 'Product Line', 'Product', 'Product Model', 'Period ID', 'PSI Type', 'QTY'];
  const row = function (psi, q) { return ['CO', 'ACC', '音频与智能配件', 'SonicBuds 6', 'M1', '2026W30', psi, q]; };
  const g = C.parseGrid([hdr, row('Sell Out', 100), row('Sell Out1', 40), row('SO TTL', 7), row('SO1', 3), row('Sell In', 50), row('Inventory', 300), row('Inventory1', 120), row('SO(TTL)', 5), row('Foo', 1)]);
  const by = {}; g.rows.forEach(function (r) { by[r.psi] = (by[r.psi] || 0) + r.qty; });
  eq(by.so, 112, 'psi: Sell Out + SO TTL + SO(TTL) → so (100+7+5), SO1 not added');
  eq(by.so1, 43, 'psi: Sell Out1 + SO1 → so1 (40+3), kept separately');
  eq(by.si, 50, 'psi: Sell In → si');
  eq(by.inv + '/' + by.inv1, '300/120', 'psi: Inventory / Inventory1');
  eq(g.report.badPsi, 1, 'psi: unknown type dropped and counted');
  ok(g.report.psiTypes && g.report.psiTypes['Sell Out1→so1'] === 1 && g.report.psiTypes['Foo→✗'] === 1, 'psi: report lists raw→mapped with counts');
  const st = C.buildStore(g.rows);
  const u = Array.from(st.units.values())[0]; const w = u.weeks.get(C.mkPeriod(2026, 30));
  eq(w.so, 112, 'store: week.so = TTL only');
...
(function () {
  ...
  eq(pv[0].si.Q2, 2000, 'fin: Q2 SI 合计');
  eq(pv[0].end.Q2, 3000, 'fin: Q2 季末库存=6 月月末库存');
  ...
  /* DOS=∞(有库存无 SO)→全额递延;缺汇率→收入 null 并标缺 */
  const rows2 = [mk(3, 0, 1000, 1000)];
  const pv2 = C.finQuarterPivot(rows2, y);
  eq(pv2[0].dos.Q1, Infinity, 'fin: 有库存无 SO → DOS ∞');
  const c2 = C.finCompute(pv2[0], pm);
  eq(c2.q.Q1.defer, 1, 'fin: DOS ∞ → 递延=全部季末库存(1K)');
  ok(c2.q.Q1.flags.indexOf('DOS∞→全额递延') >= 0, 'fin: 标注 DOS∞');
  const c3 = C.finCompute(pv[0], Object.assign({}, pm, { fx: [null, null, null, null] }));
...
(function () {
  const fw = [2026040, 2026041, 2026042, 2026043, 2026044, 2026045, 2026046];
  const raw = [100, 100, 100, 100, 3, 100, 100];
  const rawOf = function (p, i) { return { so: raw[i], src: 'ref' }; };
  /* 整桶拍数 405→410:拆分后末周 5(原值 3);比例应=410/403,不是 5/3 */
  const al = C.distribute(410, [100, 100, 100, 100, 5], 5);
  const ov = {}; al.forEach(function (v, i) { ov[fw[i]] = v; });
  const fc = C.carryTaps(fw, rawOf, ov, true);
  ok(Math.abs(fc.carryEnd - 410 / 403) < 1e-9, 'carry: 整桶比例 = Σ拍值÷Σ公式值 (' + fc.carryEnd.toFixed(4) + ')');
  ok(Math.abs(fc[5].so - 100 * 410 / 403) < 1e-9, 'carry: 后续周按整桶比例承接');
  /* 单周拍数仍=拍值÷该周公式值 */
  const fc1 = C.carryTaps(fw, rawOf, { 2026041: 250 }, true);
  eq(fc1.carryEnd, 2.5, 'carry: 单周拍 250/100 → 2.5');
  /* 拍 0 不承接 */
  const fc0 = C.carryTaps(fw, rawOf, { 2026041: 0 }, true);
  eq(fc0.carryEnd, 1, 'carry: 拍 0 → 不承接(carry 1)');
  eq(fc0[2].so, 100, 'carry: 拍 0 之后各周按公式');
  /* 清尾段:tail 标记,段后承接重置为 1 */
  const fcT = C.carryTaps(fw, rawOf, { 2026040: 300, 2026041: 900 }, true, { 2026041: 2 });
  ok(fcT[1].tail === true && fcT[1].src === 'override', 'carry: 清尾周带 tail 标记');
  eq(fcT.carryEnd, 1, 'carry: 清尾段后承接重置为 1');
  eq(fcT[2].so, 100, 'carry: 清尾后各周按公式');
  /* 公式值 0 的周被拍:不参与比例 */
  const fcZ = C.carryTaps(fw, function (p, i) { return { so: i === 1 ? 0 : 100, src: 'ref' }; }, { 2026041: 50, 2026042: 150 }, true);
  eq(fcZ.carryEnd, 1.5, 'carry: 公式值 0 的周不进比例(150/100)');
  /* simulateUnit:手拍 SO 超库存 → 进货补足,库存不负 */
  const unit = { weeks: new Map() };
  const mk = function (fcx, extra) { return Object.assign({ unit: unit, cur: { curInv: 100, curInv1: 0, windowWeeks: [] }, forecast: fcx, params: { trigger: 90, target: 120, downTarget: 60 }, cfg: { roundTo: 5 } }, extra || {}); };
  const s1 = C.simulateUnit(mk([{ p: 2026040, so: 500, src: 'override' }], { noAutoSI: true }));
  eq([s1.rows[0].so, s1.rows[0].si, s1.rows[0].siSrc, s1.rows[0].inv], [500, 400, 'tapcover', 0], 'simUnit: 手拍 500/库存 100 → SI 补 400 tapcover,库存 0');
  const s2 = C.simulateUnit(mk([{ p: 2026040, so: 500, src: 'override' }], { noAutoSI: true, eolIdx: 0 }));
  eq([s2.rows[0].so, s2.rows[0].si, s2.rows[0].capped, s2.rows[0].inv], [100, 0, true, 0], 'simUnit: 末次SI后手拍超库存 → 按可售压回并标 capped');
  const s3 = C.simulateUnit(mk([{ p: 2026040, so: 500, src: 'override' }], { noAutoSI: true, ovSI: { 2026040: 100 } }));
  eq([s3.rows[0].so, s3.rows[0].si, s3.rows[0].capped, s3.rows[0].inv], [200, 100, true, 0], 'simUnit: SI 也手拍时不补,SO 压到可售');
  const s4 = C.simulateUnit(mk([{ p: 2026040, so: 999, src: 'override', tail: true }], { noAutoSI: true, ovSI: { 2026040: 50 } }));
  eq([s4.rows[0].so, s4.rows[0].inv, s4.rows[0].tail], [150, 0, true], 'simUnit: 清尾周 SO=期初+SI,期末 0');
  const s5 = C.simulateUnit(mk([{ p: 2026040, so: 300, src: 'ref' }, { p: 2026041, so: 300, src: 'ref' }], { noAutoSI: true }));
  ok(s5.rows.every(function (r) { return r.inv >= 0; }) && s5.rows[0].capped, 'simUnit: 公式 SO 超库存也按可售压回,库存永不为负');
  /* simulateRetail:末次SI后不 tapcover */
  const r1 = C.simulateRetail({ unit: unit, cur: { curInv: 100, curInv1: 0, windowWeeks: [] }, forecast: [{ p: 2026040, so: 500, src: 'override' }], cfg: { roundTo: 5 }, eolIdx: 0, ovSI: {} });
  eq([r1.rows[0].so, r1.rows[0].buy, r1.rows[0].buySrc, r1.rows[0].capped], [100, 0, 'eol', true], 'simRetail: 末次SI后手拍超库存不补,按可售压回');
  const r2 = C.simulateRetail({ unit: unit, cur: { curInv: 100, curInv1: 0, windowWeeks: [] }, forecast: [{ p: 2026040, so: 500, src: 'override' }], cfg: { roundTo: 5 }, ovSI: {} });
  eq([r2.rows[0].so, r2.rows[0].buy, r2.rows[0].buySrc], [500, 400, 'tapcover'], 'simRetail: 未停购时手拍超库存进货补足(不变)');
  const r3 = C.simulateRetail({ unit: unit, cur: { curInv: 100, curInv1: 0, windowWeeks: [] }, forecast: [{ p: 2026040, so: 999, src: 'override', tail: true }], cfg: { roundTo: 5 }, ovSI: { 2026040: 20 } });
  eq([r3.rows[0].so, r3.rows[0].inv], [120, 0], 'simRetail: 清尾周 SO=期初+进货,期末 0');
  /* distribute 防御 */
  eq(C.distribute(-7, [1, 1], 5), [0, 0], 'distribute: 负总量不拆');
  eq(C.distribute(NaN, [1, 1], 5), [0, 0], 'distribute: 非数不拆');
  eq(C.distribute(12, [0, 0, 0, 0], 5).reduce(function (a, b) { return a + b; }, 0), 12, 'distribute: 权重全 0 仍守恒');
  /* lockSeqAlloc:规则量非步长倍数的取整不算被砍 */
  const la = C.lockSeqAlloc([{ p: 1, si: 103 }], { total: 1000, step: 5 });
  eq([la.alloc[0], la.cutAt], [100, null], 'lockSeqAlloc: 103→100 是取整不是被砍(cutAt null)');
  const lb = C.lockSeqAlloc([{ p: 1, si: 103 }, { p: 2, si: 100 }], { total: 150, step: 5 });
  eq([lb.alloc[0], lb.alloc[1], lb.cutAt], [100, 50, 2], 'lockSeqAlloc: 真被砍的周才记 cutAt');
})();
...
(function () {
  ...
  const simRows = function (so, inv0) { let inv = inv0; return fw.map(function (p) { inv = inv - so + 5; return { p: p, so: so, si: 5, inv: inv }; }); };
  const it1 = { u: u1, sim: { rows: simRows(10, 900) }, cur: { curInv: 900, dailySO: 10 / 7 } };
  const it2 = { u: u2, sim: { rows: simRows(20, 900) }, cur: { curInv: 900, dailySO: 20 / 7 } };
  const stx = { units: new Map([[u1.key, u1], [u2.key, u2]]), maxPeriod: 2026030 };
  const Dx = { fw: fw, list: [it1, it2] };
  const bctx = C.timeBuckets(Dx, stx, 'month', [2025, 2026], null, null);
  ok(bctx.bks.length > 0 && bctx.curY === 2026, 'timeBuckets: 有桶且 curY=2026');
  ok(bctx.bks.some(function (b) { return b.mode === 'sim'; }), 'timeBuckets: 今年含推演周的月桶 mode=sim');
  const vm = new Map(); Dx.list.forEach(function (it) { vm.set(it.u.key, C.unitSeries(it, bctx, 'fsd', null)); });
  const a = C.aggSeries(Dx.list, vm, bctx.bks.length);
  ...
  eq(rowY.v.so[iJan], 20 * bctx.bks[iJan].weeks.length, 'qRun: 账户 Y 行只含单元 2');
