...
- **690+ self-tests** run against the *same* core block the app executes (`/*CORE-START*/ … /*CORE-END*/` is extracted by regex, so there is zero logic duplication between app and tests).
...
Prefer a packaged build? Grab the portable **FSD-PSI-Sim.exe** from [Releases](https://github.com/lucasopsioi/psi-simulator/releases/latest) — no install, no dependencies; it opens on the built-in sample dataset so the boards are populated from the first second. **SO-Sim.exe** (same code, same engine) is the slimmed-down sell-out-planning tool: only the SO views are exposed.

## Source layout

The shipped artifact is one file, `FSD-PSI.html`, but the source of truth is `src/*.part` (55 parts listed in `src/manifest.json`). `node scripts/build.js` concatenates them back byte-for-byte; `node scripts/build.js --check` is the pre-commit gate. The engine block (`/*CORE-START*/ … /*CORE-END*/`) is what `test.js` loads, so app and tests never diverge. `FDE架构评审.md` is the architecture review that drove the V83–V90 hardening pass.
...
One build number per shipped version; see [CHANGELOG.md](CHANGELOG.md) for the condensed V10 → V90 evolution (SO-target loop, simulation engine, lock quantities, three audit rounds on hand-set numbers, board consolidation into five views, then the V83–V90 engineering hardening pass: git + tags, data contract, split sources, lineage, stable keys, error visibility, schema migrations).
