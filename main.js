...
const PROBE_FILE = argOf('--probe-file');       // 探针模式(审计用):载入 --selftest-dir 夹具+全量推演后,在页面上下文执行该 js(async 函数体,参数 ctx),结果写 <file>.result.json
/* 自测/截图跑在隔离 userData:冒烟拍数、演示剧本、夹具文件夹路径一概不许写进真实用户方案;探针按文件名各用一份 profile,可并行跑 */
if (SELFTEST_DIR || SHOT_DIR) {
  const iso = path.join(require('os').tmpdir(), PROBE_FILE ? ('fsd-psi-probe-' + path.basename(PROBE_FILE).replace(/[^\w.-]/g, '_')) : 'fsd-psi-selftest-profile');
...
function addRep(rep, r) {
  ...
  if (r.psiTypes) { rep.psiTypes = rep.psiTypes || {}; Object.keys(r.psiTypes).forEach(function (k) { rep.psiTypes[k] = (rep.psiTypes[k] || 0) + r.psiTypes[k]; }); }
}
...
ipcMain.handle('save-xlsx', async (e, payload) => {
  const safeName = String((payload && payload.name) || '导出.xlsx').replace(/[\\/:*?"<>|]/g, '_');   // Windows 文件名非法字符清洗
  ...
  try {
    ...
    ((payload && payload.sheets) || []).forEach(function (s) {
      const rowsIn = (s.rows || []).map(function (row) { return Array.isArray(row) ? row.map(function (v) { return (v === undefined || (typeof v === 'number' && !isFinite(v))) ? '' : v; }) : row; });   // undefined/NaN/Infinity → 空,避免写入失败
      const ws = getXlsx().utils.aoa_to_sheet(rowsIn);
      ...
      getXlsx().utils.book_append_sheet(wb, ws, String(s.name || 'Sheet1').replace(/[\\/:*?\[\]]/g, '_').slice(0, 31));   // sheet 名非法字符清洗
    });
    let outPath = r.filePath;
    try { getXlsx().writeFile(wb, outPath); }
    catch (err0) {
      const msg = String(err0 && err0.message || err0);
      if (/EBUSY|EPERM|EACCES|locked|being used|resource busy/i.test(msg)) {   // 目标文件正被 Excel 打开:另存为带时间戳的新文件
        const ts = new Date(); const pad = function (x) { return String(x).padStart(2, '0'); };
        const stamp = ts.getFullYear() + pad(ts.getMonth() + 1) + pad(ts.getDate()) + '-' + pad(ts.getHours()) + pad(ts.getMinutes()) + pad(ts.getSeconds());
        outPath = outPath.replace(/\.xlsx$/i, '') + '-' + stamp + '.xlsx';
        getXlsx().writeFile(wb, outPath);
        return { ok: true, path: outPath, retried: true, note: '原文件正被占用(可能在 Excel 里打开),已另存为: ' + path.basename(outPath) };
      }
      throw err0;
    }
    return { ok: true, path: outPath };
  } catch (err) { const msg = String(err && err.message || err); return { ok: false, error: msg + (/EBUSY|EPERM|EACCES/i.test(msg) ? '(文件可能正被 Excel 打开,请关闭后重试)' : '') }; }
});
...
/* CorpLink CLI 通道(移植自 销售团队 aiChatCli 原样):spawn 外部命令,stdin/file/arg 三种喂词方式;
...
function createWindow() {
  ...
  if (PROBE_FILE && SELFTEST_DIR) {
    win.webContents.on('did-finish-load', async () => {
      let result;
      try {
        const code = fs.readFileSync(PROBE_FILE, 'utf8');
        const prep = await win.webContents.executeJavaScript(
          'window.__probePrep(' + JSON.stringify({ fsd: SELFTEST_DIR, retail: SELFTEST_RETAIL, audio: SELFTEST_AUDIO }) + ')', true);
        if (!prep || !prep.ok) result = prep || { ok: false, error: 'prep failed' };
        else result = await win.webContents.executeJavaScript(
          '(async function(ctx){\n' + code + '\n})(window.__probeCtx).then(function(r){ return {ok:true,loaded:window.__probeCtx.loaded,result:r}; },function(e){ return {ok:false,error:String(e&&e.stack||e).slice(0,6000)}; })', true);
      } catch (e) { result = { ok: false, error: String(e && e.stack || e) }; }
      const out = PROBE_FILE.replace(/\.js$/i, '') + '.result.json';
      try { fs.writeFileSync(out, JSON.stringify(result, null, 1)); } catch (e) {}
      try { process.stdout.write('PROBE_RESULT ' + JSON.stringify(result).slice(0, 6000) + '\n'); } catch (e) {}
  ...
  if (SELFTEST_DIR) {
    ...
    /* 打包版跑自检时启动器先退出、stdout 拿不到:把渲染进程日志同时落到 <selftest-dir>/_selftest_run.log,
       卡住时才看得见停在哪一步(2026-09-04 门禁两次超时全靠猜) */
    const logPath = path.join(SELFTEST_DIR, '_selftest_run.log');
    const tLog0 = Date.now();
    try { fs.writeFileSync(logPath, 'START ' + new Date().toISOString() + '\n'); } catch (e) {}
    /* 主进程心跳:自检卡住时用来区分「主进程被堵」还是「渲染进程被堵」(2026-09-04) */
    const hb = setInterval(() => {
      try { fs.appendFileSync(logPath, '+' + String(Math.round((Date.now() - tLog0) / 1000)).padStart(4) + 's MAIN_ALIVE\n'); } catch (e) {}
    }, 10000);
    win.webContents.on('destroyed', () => clearInterval(hb));
    app.on('before-quit', () => clearInterval(hb));
    /* 看门狗 + 渲染进程死亡捕获:自检卡死时必须留下可诊断的结果文件,而不是静默等到外部超时
       (2026-09-04 为此白跑了几轮 25 分钟) */
    const bail = (why) => {
      try { fs.appendFileSync(logPath, 'BAIL ' + why + '\n'); } catch (e) {}
      try { fs.writeFileSync(path.join(SELFTEST_DIR, '_selftest_result.json'), JSON.stringify({ ok: false, error: why })); } catch (e) {}
      clearInterval(hb); app.exit(1);
    };
    win.webContents.on('render-process-gone', (_e3, d) => bail('renderer gone: ' + (d && d.reason) + ' exit=' + (d && d.exitCode)));
    win.on('unresponsive', () => { try { fs.appendFileSync(logPath, 'UNRESPONSIVE\n'); } catch (e) {} });
    const wd = setTimeout(() => bail('selftest watchdog timeout'), 20 * 60 * 1000);
